...
特殊方法/属性：以双下划线开头和结尾（__dunder_method__）。   
  __helloworld__ 魔术方法 
  类里的初始化方法 __init__： 类似于js 类的constructor
"""
...
class Person:
    # 魔术方法就是一些python规定好的一些特殊方法，你只要实现了它，python会在特定的场合 调用它
    def __init__(self):
        print("init")
...
print("__name__值是：" + __name__)
p1 = Person()
print("str-p1-v1的值是：" + str(p1))


# 刚才在类中定义的 __str__ 为什么在外面可以直接调用 str，原因大致类似于下面的代码
def str(obj):
    return obj.__str__()
