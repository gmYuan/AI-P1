...
with open("test_file.txt", "r", encoding="utf-8") as file:
    ...
    print(f"读取了该行的剩余内容: {repr(file.readline())}")
...
# Q: \t 是tab吗
# A: 是的同代表2个或4个空格，也是空白字符的一种，这个可以由编辑器自己定义

# Q: 中文字符是不是一个 指针推进3个或者2个
# A: 如果中文的话，一个中文汉字对应3个字节，所以索引或者说指针
#    一个汉字也会移动3位 因为这个指针指的是 字节的移动指针
#    有特别罕见的汉字也也会占用四个字节
...
# Q: 工作还得统一电脑型号吗
# A: 不需要 因为readline的话读到数据是一样的
#    read读的字符，也是一样的啊

# 既和操作系统有关，如果以编码格式GBK打开，结果又不一样
...
# Q: 还是老师说的每一行的长度加起来后，后面就是指针执行的位置
# A: 是这样的
