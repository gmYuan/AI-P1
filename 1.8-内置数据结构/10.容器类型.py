...
# 删除元素
# 删除指定值的元素，如果删除元素不存在就会报错，如 ValueError: list.remove(x): x not in list
...
# 统计元素出现次数
# 常用方法
print("my_tuple", my_tuple)  # my_tuple (1, 2, 3, 'hello', True) 
print(my_tuple.count(1))     # 2：  即 1 + true(1) = 2
...
# 添加元素
# 向集合中添加单个元素
my_set.add(6)  # list: py- append; js- push;  set: py-add;  js- add
...
# 访问元素
# 通过键访问值， 这种方法访问不存在的 key 时，会报错
name = my_dict["name"]
# name1 = my_dict["name1"]  

# 使用get方法访问（如果键不存在返回None，而不是程序报错）
age = my_dict.get("age")
