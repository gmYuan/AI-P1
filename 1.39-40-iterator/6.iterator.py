...
# 1.获取myCustomIterator.__iter__()=获取myCustomIterator
# 2.myCustomIterator.__next__()， 即：
...
# Q:  MyIterable的实现是不是就是相当于list的实现？
# A： 可以这么认为

# Q: myIterable本身就可以遍历吧
# A: 是的
#    Iterable可以被 for in 遍历:  是因为它的__iter__会返回迭代器

# Iterable(可迭代对象)   实现了__iter__方法
# Iterator(迭代器)      实现了__iter__和__next__方法
...
# Q：可迭代对象和迭代器啥区别
# A：迭代器代表一次读取过程，是一个行为/ 动作/ 功能工具
#    迭代器能执行next方法，但可迭代对象不能，或者说不一定
#    可迭代对象可以for in多次，迭代器只能for in一次
#    可迭代对象的__iter_返回的是一个新迭代器


# 每次for in 都可以有新的迭代器， 这个不一定
# 关键要看for in 的对象是谁，如果这个对象iter方法返回的是自己，那就不行了。如果返回是 新迭代器才可以


# Q: 生成器函数怎么遵循迭代协议的？
# A: 迭代协议 指的就是两个方法__iter__和__next__，只要有这二个方法就相当于遵循了迭代协议
...
# 2.不同点   可迭代对象的 __iter__方法， 返回的可能是一个别的迭代器的实例
# 2.        迭代器的的  __iter__方法， 返回的只能是自己
...
# 使用生成器表达式  创建生成器
# 语法类似于 列表推导式，但使用圆括号 而不是方括号
my_gen = (x * x for x in range(10))
print(f"生成器表达式类型: {type(my_gen)}")  # 输出: <class 'generator'>
...
for value in complex_gen:
    print(value)       # 输出: 1, 9, 25 (对应 x=0,2,4 的 x**2+2x+1)
...
print(gen.send(5))  # 10
# print(gen.send(3))      # 报错： StopIteration
...
print(next(gen))
print(gen.send(10))
# print(gen.send(50))  # 报错： StopIteration
