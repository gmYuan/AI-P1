...
print("abc" < "ab")  # False，短串排序靠前，长串更大排序靠后
...
# 如果这里把 1 该成 "1"就会报错，因为 不同同类型的不能进行比较
...
"""
# ---------------------------------------------------------------------------
# 1.22-2.1 链式比较- 本质上等价于 and，所以会有短路特性

x, y, z = 3, 7, 12

print(1 < x < 10)  # True    相当于   1 < x(3) and x(3) < 10
print(x < y < z)  # True     相当于    x(3) < y(7) and y(7) < z(12)
print(x == 3 != y)  # True   相当于  x(3) == 3 and 3 != y(7)
print(x < y > 5)  # True     相当于    x(3) < y(7) and y(7) > 5
print(5 < x > 1)  # False    因为x=3, 它不大于5
print(f"x(3) < 2 < 9的结果是: {x < 2 < 9}")  # False    因为x=3, 它不小于2
...
# 不同类型通常只能使用 等于和不等于(==与!=) 比较符号，且一般返回 False
...
# 集合和字典只支持==和!=比较
# f-string 里 {} 是占位符，要显示字面量 {1, 2} 需写成 {{1, 2}}；结果用 {(...)} 包一层
# 字典的 key 在 f-string 里用双引号，避免和外层单引号冲突；字面量用 {{"x": 1}}

print(f"{{1, 2}} == {{2, 1}} 的结果是: {({1, 2} == {2, 1})}")  # True，元素相同 无序
print(f'{{"x": 1}} == {{"x": 1}} 的结果是: {({"x": 1} == {"x": 1})}')  # True，元素相同 无序
...
# 对于集合，只能使用 == 和 !=，其他大小比较返回值都是 False，没有意义
set1 = {1}
set2 = {2}
...
"""
# ---------------------------------------------------------------------------
# 1.22-4.2  == 和is not使用场景有什么区别

str1 = ["abc"]
str2 = ["abc"]
print(id(str1), id(str2))  # 4311882816  4311983232
...
# 不能对 2个字典进行 大小比较（会报 类型错误），只能进行 ==和!= 比较
...
class Person:
    ...
    def __lt__(self, other):  # < 关系运算符的 重载
        # 小于比较：按年龄进行比较
        if isinstance(other, Person):
...
#   Person(name='Alice', age=25, salary=55000),
