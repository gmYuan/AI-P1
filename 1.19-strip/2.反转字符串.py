...
"""
# ---------------------------------------------------------------------------
# 1.19-4.1 字符反转方法 2： reversed + join


# 定义一个原始字符串
original_string = "hello"

# 使用reversed()函数 获取反向迭代器
# reversed()返回一个 反向迭代器
...
# 注意迭代器是一次性的，取完就没有了，不能再读取了   🌟🌟 
# ygm: 本质上不是没了，而是指针指到了最后一个 undefined位置了  🌟🌟 
# 所以 如果下面用 for...in 读取迭代器后，下面的 print语句打印就是空
for item in reversed_string:
   print(item)

print("迭代器是一次性的：" + "".join(reversed_string) + "\n")
...
# 列表(list)、 元组(tuple)、 集合(set)、字符串, 都是特殊的迭代器

#Q1:
# <reversed object at 0x10acb6230>,type=<class 'reversed'>，以为返回的是list类型，怎么还有reversed类型呢
# A1:
# python里有很多方法返回的都是是迭代器，而非列表，比如 reversed map filter reduce
...
# 特殊的迭代器，就是特殊在可以多次使用吗？
# A1:
# 不是的，特殊在每个迭代器都不一样。 迭代器类似于一个接口，有不同的实现



"""
# ---------------------------------------------------------------------------
# 1.19-4.1 字符反转方法 3： for循环
...
"""
# ---------------------------------------------------------------------------
# 1.19-4.1 字符反转方法 4：使用列表的 reverse()
