...
print(f"mixed_e != mixed_f的值是: {mixed_e != mixed_f}")
...
# 比较 obj_p 和 obj_q 的值是否相等
# 预期输出：True，因为它们引用的是同一个对象，值当然相等
...
if my_variable is not None:
    # 如果 my_variable 不是 None，则打印此消息
    print("Hello Variable is not None")
else:
    # 如果 my_variable 是 None，则打印此消息
    print("Hello Variable is None")


# 错误示范：使用 != None 进行比较，标准方法是 使用 is (not) None
...
print(f"str1_cached == str2_cached: {str1_cached == str2_cached}")
...
print(f"str1_uncached == str1_uncached: {str1_uncached == str1_uncached}")

# 比较 str1_uncached 和 str2_uncached 是否不是同一个对象
# 预期输出：True
print(f"str1_uncached is not str2_uncached: {str1_uncached is not str2_uncached}")
print(f"str1_uncached != str2_uncached: {str1_uncached != str2_uncached}")


# str = "h" * 1000000： 自己 逐量测试，什么量级算是 短字符串
# python None：对应 js里的 null + undefined
# python ==：  对应 js里的  ==  +  ===
...
class Person:
    ...
    def __eq__(self, other):  # 都是系统自定义的 魔术方法
        return self.id == other.id


p1 = Person("1")
p2 = Person("1")
# == 本质上就是调用 __eq__， 两者是等价的
print(p1 == p2)
print(p1.__eq__(p2))

# is 本质上就是 比较对象的内存地址(即 id 指) 是否相同， 两者是等价的
print(p1 is p2)
print(id(p1) == id(p2))
...
if p1 is None:
    print("p1 is None")
if p1 == None:
    print("p1 == None")  #会输出p1 == None，因为收到了_eq__的 自定义逻辑影响
...
p1 = Person()
p2 = Person()
print(p1 == p2)  # False, 因为内容不等
print(p1 is p2)  # False, 因为内存不等
